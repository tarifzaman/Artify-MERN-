...
export default function Explore({ isHome }) {
  ...
  const toggleFavorite = (art) => {
    ...
    let updated = isExist
      ? currentFavs.filter((f) => f._id !== art._id)
      : [...currentFavs, art];
  ...
  if (loading)
    return <div className="text-center p-20 font-bold">Loading Gallery...</div>;
      ...
      {!user && !isHome ? (
        <div className="max-w-md mx-auto text-center py-20 bg-base-200 rounded-[40px] border-2 border-dashed border-primary/30 px-10">
          ...
          <p className="text-gray-500 mb-8">
            Please login to explore our full collection of masterpieces and
            artist details.
          </p>
          <Link
            to="/login"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fadeIn">
              {displayArtworks.map((art) => {
                ...
                return (
                  <div
                    key={art._id}
                    className="card bg-base-100 shadow-lg border hover:shadow-2xl transition-all duration-300 group rounded-2xl overflow-hidden"
                  >
                    ...
                    <div className="card-body p-6 bg-white">
                      <div className="badge badge-ghost text-xs mb-1 uppercase tracking-tighter">
                        {art.category}
                      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-base-100 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl relative flex flex-col md:flex-row animate-scaleUp">
            <button
              ...
              className="absolute top-4 right-4 z-10 bg-white/20 hover:bg-black/20 p-2 rounded-full text-white md:text-black"
            >
              <FaTimes size={20} />
            </button>
            ...
            <div className="md:w-1/2 p-8 flex flex-col justify-center">
              <div className="badge badge-primary mb-4 px-4 py-3 font-bold uppercase">
              ...
              <p className="text-lg font-medium text-primary mb-4">
                Created by: {selectedArt.artist}
              </p>
              <p className="text-gray-600 mb-6 italic border-l-4 border-primary/20 pl-4">
                {selectedArt.description ||
                  "A masterfully crafted piece of art that tells a unique story."}
              </p>
              <div className="flex items-center justify-between mt-auto">
                ...
                >
                  {favorites.find((f) => f._id === selectedArt._id)
                    ? "Liked"
                    : "Add Like"}
                </button>
              </div>
